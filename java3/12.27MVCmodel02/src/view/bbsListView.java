...
//	bbsListView(List<BbsDto> list) controller에서 생성되기 때문에 매개변수를  list만들어줌
	public bbsListView(List<BbsDto> list) {
		...
		// dao를 통해서 list를 취득
//		BbsDao dao = BbsDao.getInstance(); 원래는 single턴으로 받음
//		list = dao.getBbsList();
		
		//controlloer를 통해서 넘어옴
		this.list = list;
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if(obj == writeBtn){ //controller로 이동한다음에 controller에서 보내줘야함
			this.dispose();
			//new bbsAddView();	
		}	
