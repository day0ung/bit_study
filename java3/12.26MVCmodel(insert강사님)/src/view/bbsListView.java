...
public class bbsListView extends JFrame implements MouseListener, ActionListener {
	...
	private JTextField selectField; //검색어를 입력할 수 있는 textfield
...
//			JOptionPane.showMessageDialog(null, "search button");
			...
			//무엇을 선택 ==제목,내용,작성자 //getselected index = 0,1,2,3,/ getselecteditem = 문자열
			String seledtedItem = (String)choiceList.getSelectedItem();
			JOptionPane.showMessageDialog(null,seledtedItem); //콤보박스에서 무엇이 넘어왔는지 확인
			
			//db에 접근 ->검색된 글들 뽑아오기
			//접근하기 위해서 필ㅇ한것 bbsdao
			
			BbsDao dao = BbsDao.getInstance(); //싱글턴을통해서 넘어옴
			//db에 접근해서 리스트ㅌ를 ㅂ다아옴
			list = dao.getSelectList(seledtedItem, selectField.getText());
									//검색하려고하는 컬럼
			
			//확인작업
			if(list.size() == 0 ||selectField.getText().trim().equals("")) { 				//텍스트필드의 내용이빈문자였을때
