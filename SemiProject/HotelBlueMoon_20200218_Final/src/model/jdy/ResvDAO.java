...
//		VALUES(3, 3, 3, '20200204', '20200205', SYSDATE, 50000, 0, 2);
		  String sql = " INSERT INTO RESV(SEQ, HOTELSEQ, MEMBERSEQ, ROOMSEQ, CHECKIN, CHECKOUT, RESVDATE, TOTALPRICE, CANCEL, CURRENT_GUEST, REVIEWIS, PAYMENTIS) "
	                  + " VALUES(SEQ_RESV.NEXTVAL, ?, ?, ?, ?, ?, SYSDATE, ?, 0, ?, 0, 0)  ";
		  Connection conn = null;
		  PreparedStatement psmt = null;
		  int count = 0;
	      ...
	      try {
	         ...
	         psmt.setInt(7, resv.getCurrent_guest());
	         
	         System.out.println(sql+"--addResv Insertinto--");
	...
	//add한 dto select 하기
	//hotelSeq, member.getSeq(), roomSeq, checkIn, checkOut,
...
//	and resvdate < '2020-02-28'
	...
	public void addPayment(int resvSeq) {
		//예약내역
		...
		try {
			conn = DBConnection.getConnection();
			System.out.println("1/6 addPayment success");
			
			psmt = conn.prepareStatement(sql);
			psmt.setInt(1, resvSeq);
			System.out.println("2/6 addPayment success");
			
			psmt.executeUpdate();
			System.out.println("3/6 addPayment success");
		} catch (SQLException e) {
			 System.out.println("addPayment fail");
			e.printStackTrace();
		}finally {
	         DBClose.close(psmt, conn, null);
	    }
