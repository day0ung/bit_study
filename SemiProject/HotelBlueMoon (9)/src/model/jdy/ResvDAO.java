...
public class ResvDAO {
	...
	public HotelDTO getHotelInfo(int seq) {
		...
		String sql = " SELECT SEQ, NAME, PLACE, ADDR, USE_COUNT, RATING, HOTEL_IMG " + " FROM HOTEL " + " WHERE SEQ=? ";
		...
		try {
			...
			if (rs.next()) {
				...
				hotel = new HotelDTO(rs.getInt(i++), rs.getString(i++), rs.getString(i++), rs.getString(i++),
	...
	public RoomDTO getRoomInfo(int seq) {
		...
		String sql = " SELECT SEQ, NAME, HOTELSEQ, PRICE, MAX_GUEST, ROOM_IMG " + " FROM ROOM " + " WHERE SEQ=? ";
	...
	public BM_MemberDTO getMemberInfo(String loginId) {
		...
		try {
			...
			if (rs.next()) {
				int i = 1;
				member = new BM_MemberDTO(rs.getInt(i++), rs.getString(i++), rs.getString(i++), rs.getString(i++),
	...
	public ResvDTO getResvInfo(int seq) {
		String sql = " SELECT ROOMSEQ, HOTELSEQ, CHECKIN, CHECKOut, TOTALPRICE, CURRENT_GUEST " + " FROM RESV "
				+ " WHERE SEQ=? ";
...
//				(int roomSeq, int hotelSeq, String checkIn, String checkOut, int totalPrice, int current_guest)
				resv = new ResvDTO(rs.getInt(i++), rs.getInt(i++), rs.getString(i++), rs.getString(i++), rs.getInt(i++),
						rs.getInt(i++));
	...
	public boolean checkSchedule(String checkin, String checkout, int hotelSeq, int roomSeq) {

		String sql = " SELECT SUM(USE) " + " FROM SCHEDULE " + " WHERE RESVDATE >= ? " + " AND RESVDATE < ? "
				+ " AND HOTELSEQ = ? " + " AND ROOMSEQ = ? ";
...
//		VALUES(3, 3, 3, '20200204', '20200205', SYSDATE, 50000, 0, 2);
		String sql2 = " INSERT INTO RESV(SEQ, HOTELSEQ, MEMBERSEQ, ROOMSEQ, CHECKIN, CHECKOUT, RESVDATE, TOTALPRICE, CANCEL, CURRENT_GUEST, REVIEWIS, PAYMENTIS) "
...
//	and resvdate < '2020-02-28'
	public boolean addSchedule(String checkin, String checkout, int hotelSeq, int roomSeq) {
		String sql = " UPDATE SCHEDULE  " + " SET USE = 1 " + " WHERE RESVDATE >= ? " + " AND RESVDATE < ? "
				+ " AND HOTELSEQ = ? " + " AND ROOMSEQ = ?";
		Connection conn = null;
		PreparedStatement psmt = null;
		int count = 0;
	...
	public ResvDTO selectResvAddInfo(int hotelSeq, int memberSeq, int roomSeq, String checkIn, String checkOut) {
		String sql = " SELECT SEQ, HOTELSEQ, MEMBERSEQ, ROOMSEQ, CHECKIN, CHECKOUT, TOTALPRICE, CURRENT_GUEST "
				+ " FROM RESV " + " WHERE HOTELSEQ = ? " + " AND MEMBERSEQ = ? " + " AND ROOMSEQ = ? "
		...
		try {
			...
			if (rs.next()) {
				...
				int i = 1;
				// resv = new ResvDTO(seq, hotelSeq, memberSeq, roomSeq, checkIn, checkOut,
				// totalPrice, current_guest);
				resv = new ResvDTO(rs.getInt(i++), rs.getInt(i++), rs.getInt(i++), rs.getInt(i++), rs.getString(i++),
	...
	public boolean updatePaymentIs(int resvSeq) {
		String sql = " UPDATE RESV " + " SET PAYMENTIS = 1 " + " WHERE SEQ = ? ";
