...
public class MyPageDao {
	...
	public BM_MemberDTO getMyPage(String loginId) {
		String sql = " SELECT ID, PWD, NAME, PHONENUM, EMAIL " + " FROM BM_MEMBER " + " WHERE ID=? ";
		...
		try {
			...
			if (rs.next()) {
				int i = 1;
				dto = new BM_MemberDTO(rs.getString(i++), rs.getString(i++), rs.getString(i++), rs.getString(i++),
						rs.getString(i++));
			}
	...
	public boolean myPageUpdate(String loginId, String pwd, String phoneNum, String email) {
		String sql = " UPDATE BM_MEMBER " + " SET PWD=?, PHONENUM=?, EMAIL=?" + " WHERE SEQ=? ";
...
//				(int roomSeq, int hotelSeq, String checkIn, String checkOut, int totalPrice, int current_guest)
				resv = new ResvDTO(rs.getInt(i++), rs.getInt(i++), rs.getInt(i++), rs.getInt(i++), rs.getString(i++),
						rs.getString(i++), rs.getString(i++), rs.getInt(i++), rs.getInt(i++), rs.getInt(i++),
						rs.getInt(i++));
	...
	public HotelDTO gethotelNameAddr(int seq) {
		String sql = " SELECT NAME, ADDR " + " FROM HOTEL " + " WHERE SEQ =? ";
		Connection conn = null;
		PreparedStatement psmt = null;
		ResultSet rs = null;
		...
		try {
			...
			if (rs.next()) {
				...
				hotelDto.setName(rs.getString(1));
				hotelDto.setAddr(rs.getString(2));
			}
	...
	public RoomDTO getroomNameImg(int seq) {
		String sql = " SELECT NAME, ROOM_IMG " + " FROM ROOM " + " WHERE SEQ =? ";
		Connection conn = null;
		PreparedStatement psmt = null;
		ResultSet rs = null;
		...
		try {
			...
			if (rs.next()) {
				roomDto = new RoomDTO();
				roomDto.setName(rs.getString(1));
				roomDto.setRoom_img(rs.getString(2));
			}
	...
	public String getmemName(int seq) {
		String sql = " SELECT NAME " + " FROM BM_MEMBER " + " WHERE SEQ =? ";
		Connection conn = null;
		PreparedStatement psmt = null;
		ResultSet rs = null;
		String str = "";
	...
	public boolean resvCancel(int seq) {
		String sql = " UPDATE RESV" + " SET CANCEL = 1" + " WHERE SEQ=? ";
	...
	public int getMyReviewSeq(int resvSeq) {
		String sql = " SELECT v.SEQ " + " FROM REVIEW v, RESV s " + " WHERE s.SEQ = v.RESVSEQ " + " AND s.SEQ=? ";
